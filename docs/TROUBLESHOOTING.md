...
```powershell
# Windows (PowerShell):
Get-Content migrations\006_channel_perms_ttl.sql | docker compose exec -T postgres psql -U discreet -d discreet
Get-Content migrations\007_search_privacy.sql | docker compose exec -T postgres psql -U discreet -d discreet
```

```bash
# Linux/macOS:
cat migrations/006_channel_perms_ttl.sql | docker compose exec -T postgres psql -U discreet -d discreet
cat migrations/007_search_privacy.sql | docker compose exec -T postgres psql -U discreet -d discreet
```
...
echo "SELECT tablename FROM pg_tables WHERE schemaname='public' ORDER BY tablename;" | docker compose exec -T postgres psql -U discreet -d discreet

# Check if a specific column exists:
echo "SELECT column_name FROM information_schema.columns WHERE table_name='channels' AND column_name='locked';" | docker compose exec -T postgres psql -U discreet -d discreet
```
...
```powershell
Get-Content migrations\001_schema.sql | docker compose exec -T postgres psql -U discreet -d discreet
...
```powershell
$env:DATABASE_URL = "postgres://discreet:<DB_PASSWORD>@localhost:5432/discreet"
...
docker compose exec postgres psql -U discreet -d discreet -c "SELECT 1"
# Should return: 1
...
Get-Content migrations\001_schema.sql | docker compose exec -T postgres psql -U discreet -d discreet
Get-Content migrations\002_friends.sql | docker compose exec -T postgres psql -U discreet -d discreet
