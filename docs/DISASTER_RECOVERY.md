...
docker compose exec -T postgres pg_dump -U discreet -d discreet | gzip > "$BACKUP_FILE"
...
docker run -d --name pg_verify \
  -e POSTGRES_USER=discreet \
  -e POSTGRES_PASSWORD=verify_test \
  -e POSTGRES_DB=discreet \
  postgres:16
...
gunzip -c "$BACKUP" | docker exec -i pg_verify psql -U discreet -d discreet

# 5. Verify data exists
docker exec pg_verify psql -U discreet -d discreet -c "SELECT COUNT(*) FROM users;"
# Should return a number matching your user count (approximately)

docker exec pg_verify psql -U discreet -d discreet -c "SELECT COUNT(*) FROM messages;"
...
docker compose exec postgres psql -U discreet -c "DROP DATABASE discreet;"
docker compose exec postgres psql -U discreet -c "CREATE DATABASE discreet OWNER discreet;"
...
docker compose exec postgres psql -U discreet -d discreet -c "SELECT COUNT(*) FROM users;"
...
RESTORE FROM BACKUP:
  ...
  2. gunzip -c backup.sql.gz | docker compose exec -T postgres psql -U discreet -d discreet
