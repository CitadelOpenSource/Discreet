// discreet_agent_handlers.rs — HTTP handlers for the AI Agent lifecycle.
//
// Endpoints:
...
use crate::discreet_agent_types::{
...
pub async fn search_or_spawn(
    ...
    // AI rate limit: 10/min per channel + 50/hr per user.
    crate::discreet_rate_limit::check_scoped_rate_limit(&state, &auth.user_id.to_string(), &req.server_id.to_string(), "agent_prompt")
        .await.map_err(|(_s, b)| AppError::RateLimited(format!("AI prompt rate limit. Retry after {}s", b.0["retry_after"].as_u64().unwrap_or(60))))?;
    crate::discreet_rate_limit::check_user_rate_limit(&state, &auth.user_id.to_string(), "agent_prompt_hr")
        .await.map_err(|(_s, b)| AppError::RateLimited(format!("Hourly AI prompt limit. Retry after {}s", b.0["retry_after"].as_u64().unwrap_or(3600))))?;
...
fn build_runtime_config(spec: &AgentSpecialization, config: &Config) -> AgentRuntimeConfig {
    ...
    AgentRuntimeConfig {
        backend: LLMBackend::DiscreetCloud {
...
/// Construct the agent sub-router. Merged into the main router by discreet_router.rs.
