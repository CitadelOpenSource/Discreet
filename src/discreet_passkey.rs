...
//   DELETE /auth/passkey/:id              — Remove a passkey
...
/// Return the authenticated user's registered passkeys (id, name, created_at).
pub async fn list_passkeys(
    auth: AuthUser,
    State(state): State<Arc<AppState>>,
) -> Result<impl IntoResponse, AppError> {
    let rows = sqlx::query!(
        r#"SELECT id, name, created_at FROM webauthn_credentials
...
pub async fn delete_passkey(
    ...
    tracing::info!(user_id = %auth.user_id, passkey_id = %passkey_id, "PASSKEY_DELETED");
