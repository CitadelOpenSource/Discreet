...
use discreet_server::discreet_link_preview;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Rate limiting is now Redis-backed — no in-memory cleanup needed.
    ...
    let api_v1 = axum::Router::new()
        ...
        // Passkey (WebAuthn)
        .route("/auth/passkey/list", axum::routing::get(discreet_passkey::list_passkeys))
        .route("/auth/passkey/{id}", axum::routing::delete(discreet_passkey::delete_passkey))
        ...
        // GET /servers/{server_id}/agents already registered at line 670 via discreet_agent_config_handlers::list_agents
        // ── Bots ──
        ...
        .route("/users/@me/settings", axum::routing::get(discreet_settings_handlers::get_my_settings).put(discreet_settings_handlers::patch_my_settings).patch(discreet_settings_handlers::patch_my_settings))
        ...
        // ── Link Preview ──
        .route("/link-preview", axum::routing::get(discreet_link_preview::link_preview_handler))
        ...
        // Streaming (Encrypted RTMP within MLS groups)
        ...
        .route("/admin/users/{user_id}/detail", axum::routing::get(discreet_platform_admin_handlers::user_detail))
        ...
        .route("/admin/users/{user_id}/suspend", axum::routing::post(discreet_platform_admin_handlers::suspend_user)
            .delete(discreet_platform_admin_handlers::unsuspend_user))
        .route("/admin/users/{user_id}/force-password-reset", axum::routing::post(discreet_platform_admin_handlers::force_password_reset))
        .route("/admin/users/{user_id}/flags", axum::routing::patch(discreet_platform_admin_handlers::update_user_flags))
        .route("/admin/users/{user_id}/audit", axum::routing::get(discreet_platform_admin_handlers::user_audit_log))
        .route("/admin/audit-log", axum::routing::get(discreet_platform_admin_handlers::platform_audit_log))
        // ── Lockdown ──
    ...
    // CORS: configurable via ALLOWED_ORIGINS or CORS_ORIGINS env var.
    // ALLOWED_ORIGINS takes priority over CORS_ORIGINS for backward compatibility.
    // Not set   → use APP_URL if set, otherwise localhost dev defaults
    // "*"       → allow all origins (development only)
    // URL(s)    → allow only those origins (comma-separated)
    //
    // Hardened: explicit methods, explicit headers, credentials, 1hr max-age.
    let cors_methods = [
    ...
    let cors_env = std::env::var("ALLOWED_ORIGINS")
    ...
    let cors = match cors_env.as_deref() {
        ...
        None => {
            // No origins configured — derive from APP_URL, or fall back to localhost dev defaults.
            if let Some(ref app_url) = state.config.app_url {
                tracing::info!("CORS: using APP_URL: {}", app_url);
                ...
                tracing::info!("CORS: dev defaults — localhost:5173, localhost:3001, localhost:{}", state.config.port);
                let allowed: Vec<_> = [
