...
//
// Architecture:
// 1. Standard RTMP ingest (compatible with any broadcasting software)
...
///
/// Generates a stream key derived from the user's JWT and channel ID
/// using HKDF-SHA256. The stream key authenticates the RTMP connection
/// without exposing the session token. Returns a standard RTMP URL
/// compatible with unmodified broadcasting software.
pub async fn start_stream(
    ...
    // Derive stream key: HKDF-SHA256(salt="discreet-stream-v1", ikm=user_id, info=user_id:channel_id)
...
/// Derive stream authentication key from user ID and channel ID.
/// HKDF-SHA256, domain-separated. Authenticates RTMP connections without
/// exposing the JWT. Output: 64 hex chars (256 bits).
