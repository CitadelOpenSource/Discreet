// discreet_bot_spawn_handlers.rs — AI Bot auto-spawn system.
//
// Users can spawn specialist AI bots as encrypted channel members.
// Each bot has its own cryptographic identity and participates in
// the E2EE group. The server never sees conversation content.
//
...
use crate::discreet_agent_config::{load_server_agent_config, store_encrypted_api_key};
use crate::discreet_agent_memory::{agent_memory_fact_count, build_context, clear_agent_memory};
use crate::discreet_agent_episodic_memory::{run_episodic_pipeline, EpisodicPipelineParams};
use crate::discreet_agent_provider::{AgentMessage, create_provider, strip_metadata, sanitize_agent_input, cap_response, check_agent_rate_limit};
...
/// Auto-spawn specialist AI agent as encrypted channel member.
/// Creates a bot user (if not existing), creates a private channel,
...
pub async fn prompt_bot(
    ...
    let messages: Vec<AgentMessage> = if let Ok(ref cfg) = agent_config {
        let mut msgs = build_context(
            ...
            cfg.memory_mode == crate::discreet_agent_config::MemoryMode::Summary,
            state.config.agent_key_secret.as_bytes(),
    ...
    let response_text = if let Ok(ref cfg) = agent_config {
        ...
        let (effective_provider, effective_config) = if !platform.ai_global_model.is_empty() {
            let (ptype, model_id) = match platform.ai_global_model.as_str() {
                "claude-haiku" => (crate::discreet_agent_provider::ProviderType::Anthropic, "claude-haiku-4-5-20251001".to_string()),
                "claude-sonnet" => (crate::discreet_agent_provider::ProviderType::Anthropic, "claude-sonnet-4-6".to_string()),
                "ollama-local" => (crate::discreet_agent_provider::ProviderType::Ollama, "llama3".to_string()),
