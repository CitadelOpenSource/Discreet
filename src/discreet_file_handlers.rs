...
pub async fn upload_file_blob(
    ...
    crate::discreet_rate_limit::check_user_rate_limit(&state, &auth.user_id.to_string(), "file_upload")
        .await.map_err(|(_s, b)| AppError::RateLimited(format!("File upload rate limit. Retry after {}s", b.0["retry_after"].as_u64().unwrap_or(60))))?;
