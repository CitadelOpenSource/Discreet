...
/// Read a single boolean platform setting by key. Returns the value or a default.
pub async fn get_setting_bool(state: &AppState, key: &str) -> Result<bool, AppError> {
    let row = sqlx::query!(
    ...
    Ok(row.map(|r| r.value.as_bool().unwrap_or(false)).unwrap_or(false))
}
...
pub async fn update_settings(
    ...
    // Audit log: record all settings changes.
    let changed_keys: Vec<&str> = updates.iter().map(|(k, _)| *k).collect();
    let metadata = json!({ "changed": changed_keys, "values": updates.iter().map(|(k,v)| json!({ "key": k, "value": v })).collect::<Vec<_>>() });
    if let Err(e) = sqlx::query!(
        "INSERT INTO platform_audit_log (admin_user_id, action, metadata) VALUES ($1, $2, $3)",
        caller.user_id, "settings_change", metadata,
    ).execute(&state.db).await {
        tracing::warn!(admin = %caller.user_id, "Failed to audit log settings change: {e}");
    }
