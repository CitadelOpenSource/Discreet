// discreet_agent_config.rs — Agent configuration loader and API key management.
//
...
// provider layer (`discreet_agent_provider.rs`).
//
...
//
// The agent's X25519 keypair for MLS lets it decrypt channel messages
// before sending them to the LLM. The server never sees the plaintext.
...
use crate::discreet_agent_provider::{AgentError, AgentModelConfig, ProviderType};
