// discreet_server_handlers.rs — Server (community) CRUD and membership.
//
// "Servers" in Discreet are communities (like community servers).
...
use crate::discreet_audit;
use crate::discreet_auth::AuthUser;
use crate::discreet_automod::{AutoModConfig, save_automod_config};
use crate::discreet_error::AppError;
use crate::discreet_permissions::Permission;
...
pub async fn create_server(
    ...
    crate::discreet_rate_limit::check_user_rate_limit(&state, &auth.user_id.to_string(), "create_server")
        .await.map_err(|(_s, b)| AppError::RateLimited(format!("Server creation rate limit. Retry after {}s", b.0["retry_after"].as_u64().unwrap_or(60))))?;
    ...
    // Validate name (format + profanity).
    let name = req.name.trim().to_string();
    crate::discreet_input_validation::validate_server_name(&name)?;
    crate::discreet_input_validation::validate_display_name_content(&name)?;
    ...
    if req.enable_automod {
        ...
        if let Err(e) = save_automod_config(&state.db, server_id, &automod).await {
            tracing::warn!(server_id = %server_id, error = %e, "Failed to create automod config during server creation");
        }
    }
...
pub async fn set_visibility_override(
    ...
    let actual_status = {
        ...
        map.get(&auth.user_id)
            ...
            .unwrap_or(crate::discreet_state::PresenceStatus::Online)
    };
...
pub async fn resolve_invite_code(
    ...
    crate::discreet_input_validation::validate_invite_code(&code)?;
...
pub async fn set_nickname(
    ...
    if target_user_id == auth.user_id {
        crate::discreet_permissions::require_permission(
    ...
    if let Some(ref n) = nickname {
        ...
        crate::discreet_input_validation::validate_display_name_content(n)?;
    }
...
pub async fn list_inactive_servers(
    ...
    crate::discreet_platform_admin_handlers::require_staff_role(&caller)?;
