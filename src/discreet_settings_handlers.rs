...
pub async fn get_my_settings(
    ...
    Ok(Json(UserSettingsResponse {
        ...
        show_timestamps: row.show_timestamps,
...
pub async fn patch_my_settings(
    ...
    if let Some(ref tf) = req.timestamp_format {
        if !matches!(tf.as_str(), "relative" | "12h" | "24h") {
            return Err(AppError::BadRequest("Timestamp format must be relative, 12h, or 24h".into()));
        }
    }
    if let Some(ref lm) = req.layout_mode {
        if !matches!(lm.as_str(), "simple" | "standard" | "power") {
            return Err(AppError::BadRequest("Layout mode must be simple, standard, or power".into()));
        }
    }
    ...
    let row = sqlx::query!(
        "UPDATE user_settings
         SET theme = COALESCE($1, theme),
             ...
             show_timestamps = COALESCE($24, show_timestamps),
             timestamp_format = COALESCE($25, timestamp_format),
             layout_mode = COALESCE($26, layout_mode),
             updated_at = NOW()
         WHERE user_id = $27
