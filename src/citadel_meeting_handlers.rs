// citadel_meeting_handlers.rs — Zoom-style meeting rooms with join codes.
//
...
pub async fn create_meeting(
    ...
    let pw_hash = if let Some(ref pw) = req.password {
        Some(crate::citadel_auth_handlers::hash_password(pw)?)
    } else {
        None
    };
...
pub async fn join_meeting(
    ...
    if let Some(ref hash) = m.password_hash {
        ...
        if !crate::citadel_auth_handlers::verify_password_pub(pw, hash)? {
