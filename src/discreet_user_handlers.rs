// discreet_user_handlers.rs — User profile endpoints.
//
...
pub async fn get_me(
    ...
    let user = sqlx::query!(
        "SELECT id, username, display_name, email, avatar_url, created_at, account_tier, is_guest,
                email_verified, totp_enabled, recovery_key_hash, onboarding_complete
...
/// Display name change limit: 3 per 30-day window.
const DISPLAY_NAME_MAX_CHANGES: i32 = 3;
const DISPLAY_NAME_WINDOW_DAYS: i64 = 30;
...
pub async fn update_me(
    ...
    // Validate display_name format + content.
    ...
    // Display name change rate limiting (3/month, exempt for admin/tester).
    if let Some(ref display_name) = req.display_name {
        let row = sqlx::query!(
            "SELECT display_name, account_tier, display_name_changes, display_name_reset_at
             FROM users WHERE id = $1",
            auth.user_id,
        )
        .fetch_one(&state.db)
        ...
        // Only count if the name is actually different.
        let name_changed = row.display_name.as_deref() != Some(display_name.trim());

        if name_changed {
            let exempt = matches!(row.account_tier.as_str(), "admin" | "tester");

            if !exempt {
                let now = chrono::Utc::now();
                let reset_at = row.display_name_reset_at;
                let window_expired = (now - reset_at).num_days() >= DISPLAY_NAME_WINDOW_DAYS;

                let changes = if window_expired { 0 } else { row.display_name_changes };

                if changes >= DISPLAY_NAME_MAX_CHANGES {
                    let next_reset = reset_at + chrono::Duration::days(DISPLAY_NAME_WINDOW_DAYS);
                    return Err(AppError::RateLimited(format!(
                        "You can only change your display name {} times per month. Next reset: {}",
                        DISPLAY_NAME_MAX_CHANGES,
                        next_reset.format("%B %d, %Y"),
                    )));
                }

                // Reset window if expired, then increment.
                if window_expired {
                    sqlx::query!(
                        "UPDATE users SET display_name = $1, display_name_changes = 1,
                         display_name_reset_at = NOW(), updated_at = NOW() WHERE id = $2",
                        display_name.trim(),
                        auth.user_id,
                    )
                    .execute(&state.db)
                    ...
                    sqlx::query!(
                        "UPDATE users SET display_name = $1, display_name_changes = display_name_changes + 1,
                ...
                // Exempt user — just update, no counting.
                sqlx::query!(
            ...
            // Broadcast display name change to all servers the user is a member of.
            ...
            for sid in server_ids {
                state.ws_broadcast(sid, event.clone()).await;
            }
        }
        // If name didn't change, skip all the above.
    }
    ...
    // One-way: onboarding_complete can only be set to true, never reverted
    if req.onboarding_complete == Some(true) {
        sqlx::query!(
    ...
    // Return updated profile with change count.
    let user = sqlx::query!(
        "SELECT id, username, display_name, email, avatar_url, created_at, account_tier, is_guest,
                email_verified, totp_enabled, recovery_key_hash, display_name_changes, display_name_reset_at,
                onboarding_complete
         FROM users WHERE id = $1",
        auth.user_id,
    )
    ...
    Ok(Json(serde_json::json!({
        ...
        "display_name_reset_at": user.display_name_reset_at.to_rfc3339(),
        "onboarding_complete": user.onboarding_complete,
    })))
}
...
pub async fn delete_account(
    ...
    use crate::discreet_auth_handlers::require_reauth;
    ...
    if !crate::discreet_auth_handlers::verify_password_pub(&req.password, &stored_hash)? {
...
pub async fn change_email(
    ...
    use crate::discreet_auth_handlers::{generate_hex_token, require_reauth, verify_password_pub};
    use crate::discreet_email_handlers::send_verification_link_email;
...
pub async fn update_status(
    ...
    // Validate lengths + control characters.
    if let Some(ref s) = req.status {
        crate::discreet_input_validation::validate_custom_status(s)?;
    }
    ...
    if let Some(ref p) = req.presence {
        use crate::discreet_state::PresenceStatus;
