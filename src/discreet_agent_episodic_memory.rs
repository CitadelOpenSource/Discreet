// discreet_agent_episodic_memory.rs — Encrypted persistent agent memory.
//
// WHAT THIS IS:
...
//
// The agent's learned knowledge is encrypted with its own key and stored
// as ciphertext the server cannot read. The agent decrypts both conversation
// history and persistent memory using its key — zero-knowledge personalization.
//
...
//   Tier 1 — Working Memory: sliding window (discreet_agent_memory.rs)
...
use crate::discreet_agent_provider::{AgentError, AgentMessage, AgentModelConfig, LlmProvider};
