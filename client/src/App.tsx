...
import { T, getInp, btn, setTheme, ta, applyServerTheme, registerThemeSync, FONT_SIZE_MAP } from './theme';
...
import { kernelInit, kernelGenerateOutgoing, kernelDecrypt } from './kernel/kernelClient';
import type { RenderMessage } from './kernel/types.generated';
...
import { useDebugMode, DebugOverlay, DebugStyles } from './components/DebugOverlay';
import { VoicePanel } from './components/VoicePanel';
import { MessageList } from './components/MessageList';
import { MessageInput } from './components/MessageInput';
import type { ParentMsg } from './components/ThreadView';
...
const DiscoverPanel       = lazy(() => import('./components/DiscoverPanel').then(m => ({ default: m.DiscoverPanel })));
...
const VideoGrid           = lazy(() => import('./components/VideoGrid').then(m => ({ default: m.VideoGrid })));
...
const VoiceConfirmationModal = lazy(() => import('./components/voice/VoiceConfirmationModal').then(m => ({ default: m.VoiceConfirmationModal })));
const AddPeopleModal      = lazy(() => import('./components/voice/AddPeopleModal').then(m => ({ default: m.AddPeopleModal })));
const QrScanner           = lazy(() => import('./components/QrScanner').then(m => ({ default: m.QrScanner })));
const LeaderboardPanel    = lazy(() => import('./components/Gamification').then(m => ({ default: m.LeaderboardPanel })));
...
const WarrantCanary       = lazy(() => import('./components/legal/WarrantCanary').then(m => ({ default: m.WarrantCanary })));
...
function GlobalStyles() {
  return (
    <style>{`
      ...
      ::-webkit-scrollbar { width: var(--scrollbar-width, 4px); }
      ::-webkit-scrollbar-track { background: transparent; }
      ::-webkit-scrollbar-thumb { background: var(--scrollbar-color, rgba(255,255,255,0.06)); border-radius: var(--radius-sm, 4px); }
      ::-webkit-scrollbar-thumb:hover { background: var(--border-color, rgba(255,255,255,0.14)); }
      * { scrollbar-width: thin; scrollbar-color: var(--scrollbar-color, rgba(255,255,255,0.06)) transparent; }
      ...
      .bubbles-on .msg-bubble-self { background: var(--bubble-self-bg, rgba(124,58,237,0.12)); margin-left: auto; border-bottom-right-radius: 4px; }
...
const InlineLoader = <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: T.mt, fontSize: 13 }}>Loading...</div>;
...
function ModalLoadingFallback() {
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 9998, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: T.sf, border: `1px solid ${T.bd}`, borderRadius: 14, padding: 32, width: 420 }}>
...
function ColorTool({ onInsert }: { onInsert: (v: string) => void }) {
  const [hex, setHex] = React.useState(T.ac);
...
export default function App() {
  ...
  useEffect(() => {
    ...
    api.getSettings?.().then((s: any) => {
      ...
      if (s) {
        ...
        if (s.layout_mode && (s.layout_mode === 'simple' || s.layout_mode === 'standard' || s.layout_mode === 'power')) { layoutMode.setMode(s.layout_mode); }
        ...
        // Sync font_size (text size preset) from server
        if (s.font_size) {
          document.documentElement.style.setProperty('--app-font-size', FONT_SIZE_MAP[s.font_size] || '14px');
          localStorage.setItem('d_font_size', s.font_size);
        }
        // Sync accessibility settings from server
        if (s.high_contrast !== undefined) { localStorage.setItem('d_high_contrast', String(!!s.high_contrast)); setA11yHighContrast(!!s.high_contrast); }
        if (s.reduce_motion !== undefined) { localStorage.setItem('d_reduce_motion', String(!!s.reduce_motion)); setA11yReduceMotion(!!s.reduce_motion); }
        if (s.focus_rings !== undefined) { localStorage.setItem('d_focus_rings', String(!!s.focus_rings)); setA11yFocusRings(!!s.focus_rings); }
        // Sync voice preferences from server
        if (s.voice_mode) localStorage.setItem('d_vmode', s.voice_mode);
        if (s.voice_noise_suppression !== undefined) localStorage.setItem('d_noiseSup', String(s.voice_noise_suppression));
        if (s.push_to_talk_key) localStorage.setItem('d_pttkey', s.push_to_talk_key);
        // Sync notification master switch from server
        if (s.sounds_enabled !== undefined) localStorage.setItem('d_sounds', String(s.sounds_enabled));
    ...
    const handler = (evt: any) => {
      ...
      if (evt.type === 'ws_status') {
        if (evt.status === 'connected') {
          ...
          if (wsHadDisconnect.current && curChannelRef.current) loadMessages(curChannelRef.current);
  ...
  const loadMessages = async (ch: Channel | null) => {
    ...
    try {
      ...
      const decrypted = await Promise.all(raw.map(async (m: any) => {
        ...
        const result = await dec(ch.id, m.content_ciphertext).catch(() => ({ text: m.content_ciphertext } as DecResult));
        return { ...m, text: result.text, renderModel: result.renderModel, authorName: userMap[m.author_id] || 'Unknown' };
      }));
      setMessages(decrypted.reverse());
    } catch { /* load failed — user sees empty state */ } finally { setLoadingMessages(false); }
  };
  ...
  const isCompact = isMobile || layoutMode.isSimple;

  // Bottom tab handler — maps tabs to existing view system (Simple + mobile)
  const handleMobileTab = (tab: MobileTab) => {
    setMobileTab(tab);
    if (tab === 'chats') { setView('dm'); setMobileMenuOpen(false); }
    else if (tab === 'servers') { setView('home'); setHomeTab('home'); setMobileMenuOpen(false); }
    else if (tab === 'friends') { goHome(); setHomeTab('friends'); setMobileMenuOpen(false); }
  ...
  const createServer = async () => {
    ...
    try {
      ...
      if (s?.error) {
        if (s.error?.code === 'TIER_LIMIT' || s.error === 'TIER_LIMIT') { handleTierLimitError(s); return; }
        const msg = typeof s.error === 'string' ? s.error : s.error?.message || 'Failed to create server';
  ...
  const openMemberCtx = (e: React.MouseEvent, uid: string) => {
    ...
    const items: CtxMenuItem[] = [
      ...
      ...(localStorage.getItem('d_dev_tools') === 'true' ? [{ label: 'Copy User ID', icon: <I.Copy />, fn: () => navigator.clipboard?.writeText(uid) }] : []),
    ];
  ...
  if (pathname === '/app/terms' || pathname === '/terms') return <Suspense fallback={InlineLoader}><TermsOfService /></Suspense>;
  if (pathname === '/app/privacy' || pathname === '/privacy') return <Suspense fallback={InlineLoader}><PrivacyPolicy /></Suspense>;
  if (pathname === '/download' || pathname === '/app/download') return <Suspense fallback={InlineLoader}><DownloadPage /></Suspense>;
  if (pathname === '/canary' || pathname === '/app/canary') return <Suspense fallback={InlineLoader}><WarrantCanary /></Suspense>;
  ...
  const renderServerIcon = (s: Server, isFav: boolean) => {
    ...
    <div key={s.id} onClick={() => selectServer(s)}
      ...
      title={s.name}
      style={{ width: 48, height: 48, borderRadius: isActive ? 'var(--radius-lg, 16px)' : 'var(--radius-full, 24px)', background: s.icon_url ? 'transparent' : (isActive ? `linear-gradient(135deg,${T.ac},${T.ac2})` : T.sf2), display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'border-radius .2s, box-shadow .15s ease', fontSize: 16, fontWeight: 700, color: isActive ? '#fff' : T.tx, overflow: 'hidden', position: 'relative' }}>
      ...
      {hasUnread && !isActive && (
        <div style={{ position: 'absolute', top: -4, right: -4, minWidth: 16, height: 16, borderRadius: 'var(--radius-md)', background: T.err, border: `2px solid ${T.bg}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#fff', padding: '0 3px' }}>
          {srvUnreadCount > 99 ? '99+' : srvUnreadCount}
        </div>
      )}
      {serverNotifLevels[s.id] === 'nothing' && (
        <div style={{ position: 'absolute', bottom: -2, right: -2, width: 16, height: 16, borderRadius: 'var(--radius-md)', background: T.bg, border: `1px solid ${T.bd}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9 }} title="Muted"><I.BellOff s={9} /></div>
      )}
    </div>
    );
  };
  ...
  // ══════════════════════════════════════════════════════════
  return (
    <div className={`chat-root layout-${layoutMode.mode} density-${msgDensity}${chatBubbles ? ' bubbles-on' : ''}${chatBubbles && bubblePosition === 'aligned' ? ' bubbles-aligned' : ''}${nightActive ? ' nighttime-active' : ''}${a11yReduceMotion ? ' reduce-motion' : ''}${a11yHighContrast ? ' high-contrast' : ''}${a11yFocusRings ? ' focus-visible' : ''}`} style={{ color: T.tx, fontFamily: 'var(--font-primary)', background: a11yHighContrast ? '#000' : T.bg, paddingTop: (isMobile && showAppBanner ? 40 : 0) + (me && !me.email_verified && me.email && !verifyBannerDismissed && _storage.getItem('d_verify_skipped') === '1' ? 44 : 0), paddingBottom: (isCompact ? 56 : 0) + (voiceChannel ? 48 : 0) }}>
      <GlobalStyles />
      <DebugStyles />
      {debugMode && <DebugOverlay />}
      {/* Mobile app download banner */}
      {isMobile && showAppBanner && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1001, height: 40, background: T.ac, display: 'flex', alignItems: 'center', padding: '0 12px' }}>
      ...
      {/* Verify email banner — fixed bar at top, shown when user skipped verification */}
      ...
      <div className="server-rail" role="navigation" aria-label="Server list" data-testid="server-list" data-component="ServerRail" style={{ width: 72, minWidth: 72, background: T.bg, display: isCompact ? 'none' : 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 0', gap: 4, borderRight: `1px solid ${T.bd}`, overflowY: 'auto' }}>
        ...
        <div title="Direct Messages" role="button" aria-label="Direct Messages" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') setView('dm'); }} style={{ width: 48, height: 48, borderRadius: view === 'dm' ? 16 : 24, background: view === 'dm' ? `linear-gradient(135deg,${T.ac},${T.ac2})` : T.sf2, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'border-radius .2s', color: view === 'dm' ? '#000' : T.tx, position: 'relative', fontSize: 14 }} onClick={() => { setView('dm'); }}>
          <I.Msg />
          {totalDmUnread > 0 && <div style={{ position: 'absolute', top: -2, right: -2, minWidth: 16, height: 16, borderRadius: 'var(--border-radius)', background: '#ed4245', color: '#fff', fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `2px solid ${T.bg}`, padding: '0 3px' }}>{totalDmUnread}</div>}
        </div>
        <div style={{ width: 28, height: 2, background: T.bd, borderRadius: 1, margin: '4px 0' }} />
        {/* Favorite servers */}
        {(() => {
        ...
        {showServersSkeleton && servers.length === 0
          ? Array.from({ length: 4 }).map((_, i) => (
              <div key={i} style={{ width: 48, height: 48, borderRadius: 'var(--radius-full, 24px)', ...shimBase, animationDelay: `${i * 0.1}s` }} />
            ))
          : (() => {
        ...
        <div onClick={() => me?.is_guest ? setUpgradeFeature('create servers') : setModal('create-server')} title="Create Server" role="button" aria-label="Create Server" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') { me?.is_guest ? setUpgradeFeature('create servers') : setModal('create-server'); } }} style={{ width: 48, height: 48, borderRadius: 'var(--radius-full, 24px)', background: T.sf2, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: T.ac, fontSize: 20 }}>+</div>
      </div>
      ...
      <SectionBoundary name="sidebar">
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px 6px' }}>
          {view !== 'server' && (<>
            <div onClick={goHome} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 14, fontWeight: 600, color: homeTab === 'home' ? T.ac : T.mt, background: homeTab === 'home' ? 'rgba(124,58,237,0.08)' : 'transparent' }}><I.Home /> {t('nav.home')}</div>
            <div onClick={() => { goHome(); setHomeTab('friends'); }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 14, fontWeight: 600, color: homeTab === 'friends' ? T.ac : T.mt, background: homeTab === 'friends' ? 'rgba(124,58,237,0.08)' : 'transparent' }}><I.Users /> {t('nav.friends')}</div>
            ...
            {dms.map(dm => (
              <div key={dm.id} onClick={() => selectDm(dm)} onContextMenu={(e) => {
                ...
                setCtxMenu({ x: e.clientX, y: e.clientY, items: [
                  ...
                  ...(localStorage.getItem('d_dev_tools') === 'true' ? [{ label: 'Copy User ID', icon: <I.Copy />, fn: () => navigator.clipboard?.writeText(dm.other_user_id) }] : []),
                ]});
              }} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 13, color: curDm?.id === dm.id ? T.ac : T.mt, background: curDm?.id === dm.id ? 'rgba(124,58,237,0.08)' : 'transparent' }}>
            ...
            {groupDms.map(gdm => (
              <div key={gdm.id} onClick={() => selectGroupDm(gdm)} onContextMenu={e => { e.preventDefault(); setCtxMenu({ x: e.clientX, y: e.clientY, items: [{ label: 'Leave Group', icon: <I.Out />, danger: true, fn: () => { setGroupDms(p => p.filter(g => g.id !== gdm.id)); if (curGroupDm?.id === gdm.id) goHome(); } }] }); }}
                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 6, cursor: 'pointer', fontSize: 13, color: curGroupDm?.id === gdm.id ? T.ac : T.mt, background: curGroupDm?.id === gdm.id ? 'rgba(124,58,237,0.08)' : 'transparent' }}>
        ...
        {/* Bug Report + Sign Out — above User Bar */}
        <div style={{ padding: '4px 10px', display: 'flex', gap: 6 }}>
          <div onClick={() => setShowBugReport(true)} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '5px 0', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontWeight: 600, color: T.mt, background: T.sf2, border: `1px solid ${T.bd}` }}
            onMouseEnter={e => (e.currentTarget.style.color = T.tx)} onMouseLeave={e => (e.currentTarget.style.color = T.mt)}>
            <I.Flag s={12} /> Report Bug
          </div>
          <div onClick={async () => {
            if (!confirm('Are you sure you want to sign out?')) return;
            try { await api.logout(); } catch {}
            window.location.href = '/';
          }} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '5px 0', borderRadius: 6, cursor: 'pointer', fontSize: 11, fontWeight: 600, color: T.mt, background: T.sf2, border: `1px solid ${T.bd}` }}
            onMouseEnter={e => (e.currentTarget.style.color = '#ff4757')} onMouseLeave={e => (e.currentTarget.style.color = T.mt)}>
            <I.Out s={12} /> Sign Out
          </div>
        </div>
      ...
      <div className="chat-main" role="main" aria-label="Chat" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', paddingBottom: isMobile ? 72 : 0 }}>
        {/* Connection status is shown via the header wifi icon */}
        {announcement && (
        ...
        <div className="chat-header" style={{ padding: '10px 16px', borderBottom: `1px solid ${T.bd}`, display: 'flex', alignItems: 'center', gap: 10, minHeight: 48 }}>
          ...
          {layoutMode.isSimple && (view === 'server' || view === 'dm') && (
            <button
              onClick={() => { if (view === 'dm') { handleMobileTab('chats'); } else { handleMobileTab('servers'); } }}
              aria-label="Back"
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.ac, padding: 4, display: 'flex', alignItems: 'center', minHeight: 44, minWidth: 44 }}
            >
              <I.ChevL s={22} />
            </button>
          )}
          {/* Hamburger — mobile only (non-Simple) */}
          {!layoutMode.isSimple && (
            <button
          ...
          {view === 'server' && curChannel && (<><I.Hash s={18} /><span style={{ fontWeight: 700, fontSize: 15 }} title={layoutMode.isPower ? `ch:${curChannel.id}` : undefined}>{curChannel.name}</span>
            ...
            <div style={{ cursor: 'pointer', color: T.mt, padding: '2px 6px', display: 'flex', alignItems: 'center' }} title="Reminders" aria-label="Reminders">
              <I.Clock s={14} />
            </div>
          </>)}
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <div title={wsStatus === 'connected' ? 'Connected' : wsStatus === 'reconnecting' ? 'Reconnecting...' : 'Disconnected'}
              style={{ padding: 4, display: 'flex', alignItems: 'center', color: wsStatus === 'connected' ? '#43b581' : wsStatus === 'reconnecting' ? '#faa61a' : '#f04747', ...(wsStatus === 'reconnecting' ? { animation: 'wsPulse 1.5s ease-in-out infinite' } : {}) }}>
              {wsStatus === 'disconnected' ? <I.WifiOff s={14} /> : <I.Wifi s={14} />}
            </div>
            ...
            {/* Upgrade button */}
            <button
              onClick={() => { setToast('Pro plans coming soon!'); setTimeout(() => setToast(''), 3000); }}
              title="Coming Soon"
              style={{
                padding: '4px 12px', borderRadius: 6, fontSize: 12, fontWeight: 600,
                background: 'transparent', border: `1px solid ${ta(T.ac,'55')}`,
                color: T.ac, cursor: 'pointer', whiteSpace: 'nowrap',
              }}
            >
              Upgrade
            </button>
          ...
          <div data-testid="home-page" data-component="HomePage" style={{ flex: 1, overflowY: 'auto', padding: '24px 20px' }}>
            ...
            <div style={{ textAlign: 'center', padding: '32px 20px 16px', marginBottom: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
                <button onClick={() => me?.is_guest ? setUpgradeFeature('create servers') : setModal('create-server')} style={{ padding: '11px 26px', borderRadius: 10, border: 'none', background: `linear-gradient(135deg,${T.ac},${T.ac2})`, color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>{t('server.create')}</button>
                <button onClick={() => setModal('join-server')} style={{ padding: '11px 26px', borderRadius: 10, border: `1px solid ${T.bd}`, background: T.sf, color: T.tx, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>Join Server</button>
              </div>
            </div>
            {/* Servers Grid */}
            {servers.length > 0 && (<>
            ...
            {dms.length > 0 && (<>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {dms.slice(0, 5).map(dm => (
                  <div key={dm.id} onClick={() => selectDm(dm)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: T.sf, borderRadius: 'var(--border-radius)', cursor: 'pointer', border: `1px solid ${T.bd}` }}>
            ...
            <div style={{ display: 'flex', gap: 16, marginTop: 24, flexWrap: 'wrap' }}>
              ...
              {React.createElement(function CalendarWidget() {
                ...
                return (
                  <div style={{ flex: 1, minWidth: 260, background: T.sf, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}`, padding: 16 }}>
                ...
                return (
                  <div style={{ flex: 1, minWidth: 260, background: T.sf, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}`, padding: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
                      ...
                      <div onClick={add} style={{ padding: '6px 10px', background: T.ac, color: '#fff', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>+</div>
                    </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
              {[
                ...
                <div key={bot.name} onClick={() => { setToast(`${bot.name} — activate from a server's Bot Marketplace`); setTimeout(() => setToast(''), 3000); }} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: T.sf, borderRadius: 10, cursor: 'pointer', border: `1px solid ${T.bd}` }}
                  ...
                  <div style={{ width: 36, height: 36, borderRadius: 'var(--border-radius)', background: `${bot.color}22`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>{bot.icon}</div>
        ...
        {view === 'home' && homeTab === 'friends' && (
          <Suspense fallback={InlineLoader}><FriendsView
            ...
            isGuest={me?.is_guest}
            isMobile={isMobile}
            onStartCall={async (userId: string) => {
              try {
        ...
        {view === 'home' && homeTab === 'events' && curServer && (
          <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
            <Suspense fallback={InlineLoader}><EventsPanel serverId={curServer.id} isOwner={hasPrivilege(myPrivilege, PRIVILEGE_LEVELS.ADMIN)} channels={channels} /></Suspense>
          </div>
        )}
        {view === 'home' && homeTab === 'events' && !curServer && (
        ...
        {view === 'home' && homeTab === 'tools' && React.createElement(function ToolsView() {
          ...
          const btnS = (bg: string, c: string) => ({ padding: '16px 0', borderRadius: 'var(--border-radius)', cursor: 'pointer', fontSize: 18, fontWeight: 600 as const, background: bg, color: c, border: 'none', textAlign: 'center' as const });
            ...
            <div style={{ flex: 1, padding: 24, overflowY: 'auto' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
                {([
                  ...
                  <div key={t.key} onClick={() => t.action ? t.action() : setActiveTool(t.key)} style={{ padding: 16, background: T.sf, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}`, cursor: 'pointer', textAlign: 'center' }}
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
              ...
              {activeTool === 'calc' && (
                <div style={{ width: 300, background: T.sf, borderRadius: 16, border: `1px solid ${T.bd}`, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
                  <div style={{ padding: '24px 20px 16px', textAlign: 'right', fontSize: 36, fontWeight: 300, fontFamily: 'var(--font-mono)', color: T.tx, overflow: 'hidden', textOverflow: 'ellipsis' }}>{display}</div>
              ...
              {activeTool === 'unit' && (
                <div style={{ width: 320, background: T.sf, borderRadius: 16, border: `1px solid ${T.bd}`, padding: 20 }}>
                  ...
                  <select value={unitType} onChange={e => setUnitType(e.target.value)} style={{ width: '100%', padding: '8px 12px', background: T.bg, border: `1px solid ${T.bd}`, borderRadius: 'var(--border-radius)', color: T.tx, fontSize: 13, marginBottom: 10 }}>
                  ...
                  <div style={{ padding: '12px 16px', background: T.bg, borderRadius: 'var(--border-radius)', fontSize: 16, fontWeight: 600, textAlign: 'center', fontFamily: 'var(--font-mono)' }}>{unitVal ? convertUnit() : 'Enter a value above'}</div>
                </div>
              )}
              ...
              {activeTool === 'timer' && React.createElement(function StopwatchTool() {
                ...
                return (
                  <div style={{ width: 300, background: T.sf, borderRadius: 16, border: `1px solid ${T.bd}`, padding: 20, textAlign: 'center' }}>
                    <div style={{ fontSize: 48, fontWeight: 300, fontFamily: 'var(--font-mono)', marginBottom: 20 }}>{fmt(elapsed)}</div>
                    <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                      <button onClick={() => setRunning(!running)} style={{ ...btn(true), padding: '8px 24px' }}>{running ? 'Stop' : 'Start'}</button>
                      <button onClick={() => { setRunning(false); setElapsed(0); }} style={{ padding: '8px 24px', background: T.sf2, color: T.mt, border: `1px solid ${T.bd}`, borderRadius: 'var(--border-radius)', cursor: 'pointer' }}>Reset</button>
                    </div>
                  </div>
                );
              })}
        ...
        {view === 'home' && homeTab === 'discover' && (
          <Suspense fallback={InlineLoader}><DiscoverPanel
        ...
        {/* ─── Admin Dashboard ─── */}
        {view === 'home' && homeTab === 'admin' && (isPlatformDevOrAdmin || isPlatformTester) && (
          <SectionBoundary name="admin"><Suspense fallback={InlineLoader}><AdminDashboard platformUser={platformUser} /></Suspense></SectionBoundary>
        )}
        ...
        {view === 'dm' && curGroupDm && (<>
          ...
          <div style={{ padding: '10px 16px', borderTop: `1px solid ${T.bd}` }}>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <div onClick={sendMessage} role="button" aria-label="Send message" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') sendMessage(); }} style={{ padding: '8px 14px', background: `linear-gradient(135deg,${T.ac},${T.ac2})`, borderRadius: 'var(--border-radius)', cursor: 'pointer', color: '#fff', fontWeight: 700, fontSize: 13 }}>Send</div>
            </div>
          </div>
        </>)}
        ...
        {view === 'dm' && curDm && (<>
          ...
          <div style={{ padding: '10px 16px', borderTop: `1px solid ${T.bd}` }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <input ref={inputRef} value={msgInput} onChange={e => setMsgInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); sendMessage(); } }} placeholder={`Message ${curDm.other_username}`} style={{ flex: 1, padding: '10px 14px', background: T.sf2, border: `1px solid ${T.bd}`, borderRadius: 'var(--border-radius)', color: T.tx, fontSize: 14, outline: 'none', fontFamily: 'var(--font-primary)' }} />
              <div onClick={sendMessage} role="button" aria-label="Send message" tabIndex={0} onKeyDown={e => { if (e.key === 'Enter') sendMessage(); }} style={{ padding: '8px 14px', background: `linear-gradient(135deg,${T.ac},${T.ac2})`, borderRadius: 'var(--border-radius)', cursor: 'pointer', color: '#fff', fontWeight: 700, fontSize: 13 }}>Send</div>
            </div>
          </div>
        </>)}
        ...
        {view === 'server' && curChannel && (<div key={channelFadeKey} style={{ display: 'contents', animation: 'fadeIn 0.2s ease' }}>
          ...
          {vc.screenSharing && (
            <div style={{
              ...
              border: '1px solid rgba(255,71,87,0.3)', borderRadius: 'var(--radius-md)', marginBottom: 6,
            }}>
              <I.Monitor s={14} />
          ...
          {voiceChannel && (vc.videoEnabled || vc.screenSharing || vc.streams.size > 0) && (
            <Suspense fallback={InlineLoader}><VideoGrid
      ...
      {view === 'server' && openThread && curChannel && (
        <Suspense fallback={InlineLoader}><ThreadView
      ...
      {/* ═══ Modals ═══ */}
      {modal === 'create-server' && (
        <Modal title={t('server.create')} onClose={() => { setModal(null); setServerPreset(null); }} wide>
          <div style={{ padding: 20 }}>
            {!serverPreset && (<>
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 16 }}>Choose a template or start from scratch</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10, marginBottom: 20 }}>
                {[
                  { key: 'quick', icon: <I.Zap s={20} />, name: t('server.quickCreate'), desc: 'Just a name', color: T.ac },
                  { key: 'gaming', icon: <I.Gamepad s={20} />, name: t('server.gaming'), desc: 'Lobby + GameMaster bot', color: '#5865F2' },
                  { key: 'meeting', icon: <I.Clipboard s={20} />, name: t('server.work'), desc: 'Agenda + MeetingBot', color: '#faa61a' },
                  { key: 'community', icon: <I.Users s={20} />, name: t('server.community'), desc: 'Welcome + CommunityBot', color: '#3ba55d' },
                ].map(p => (
                  <div key={p.key} onClick={() => p.key === 'quick' ? setServerPreset('quick') : setServerPreset(p.key)} style={{ padding: '16px 12px', borderRadius: 10, border: `2px solid ${T.bd}`, cursor: 'pointer', textAlign: 'center' }}
                    ...
                    onMouseLeave={e => e.currentTarget.style.borderColor = T.bd}>
                    <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 6, color: p.color }}>{p.icon}</div>
            ...
            {serverPreset && (<>
              ...
              <input style={{ ...getInp(), marginBottom: 12 }} value={createName} onChange={e => setCreateName(e.target.value)} placeholder={serverPreset === 'gaming' ? 'Epic Gamers' : serverPreset === 'meeting' ? 'Team Standup' : 'My Server'} autoFocus onKeyDown={e => { if (e.key === 'Enter') createServer(); }} />
              {serverPreset !== 'quick' && (
                <div style={{ padding: '10px 12px', background: T.sf2, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}`, marginBottom: 12, fontSize: 12, color: T.mt }}>
                  ✨ This template will auto-create channels and spawn an AI bot for your server.
                </div>
              )}
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', background: T.sf2, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}`, marginBottom: 12, cursor: 'pointer', fontSize: 13 }}>
              ...
              <button onClick={createServer} style={btn(!!createName.trim())}>{t('server.create')}</button>
            </>)}
          </div>
        </Modal>
      )}
            ...
            <div onClick={e => e.stopPropagation()} style={{ width: 300, background: T.sf, borderRadius: 10, border: `1px solid ${T.bd}`, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '14px 14px 10px', borderBottom: `1px solid ${T.bd}` }}>
                ...
                <input
                  ...
                  style={{ width: '100%', boxSizing: 'border-box', padding: '8px 10px', background: T.sf2, border: `1px solid ${T.bd}`, borderRadius: 6, color: T.tx, fontSize: 13, outline: 'none', fontFamily: 'var(--font-primary)' }}
                />
              </div>
                ...
                {gdmFriends.map((f: any) => {
                  ...
                  return (
                    <div key={uid} onClick={() => setGdmSelected(p => checked ? p.filter(id => id !== uid) : [...p, uid])}
                      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 'var(--border-radius)', cursor: 'pointer', border: `1px solid ${checked ? T.ac : T.bd}`, background: checked ? `${ta(T.ac,'12')}` : 'transparent', transition: 'border-color .1s' }}>
      ...
      {modal === 'join-server' && (
        <Modal title="Join Server" onClose={() => { setModal(null); setShowQrScanner(false); }}>
          <div style={{ padding: 20 }}>
            {showQrScanner ? (
              <Suspense fallback={InlineLoader}><QrScanner
                onScan={(data) => {
              ...
              <>
                ...
                <div style={{ display: 'flex', gap: 8, marginBottom: 0 }}>
                  ...
                  <button onClick={() => setShowQrScanner(true)} style={{ padding: '9px 14px', fontSize: 12, fontWeight: 600, borderRadius: 'var(--radius-md)', cursor: 'pointer', background: T.sf2, color: T.tx, border: `1px solid ${T.bd}`, whiteSpace: 'nowrap' }}>
                    Scan QR
                  </button>
                </div>
              </>
      ...
      {connectAction && (
        <Modal title={connectAction.type === 'friend' ? 'Add Friend' : 'Join Server'} onClose={() => setConnectAction(null)}>
          <div style={{ padding: 24, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button onClick={() => setConnectAction(null)} style={{ padding: '10px 20px', borderRadius: 'var(--radius-md)', border: `1px solid ${T.bd}`, background: T.sf2, color: T.mt, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
              <button onClick={executeConnect} style={{ padding: '10px 20px', borderRadius: 'var(--radius-md)', border: 'none', background: `linear-gradient(135deg,${T.ac},${T.ac2})`, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
        ...
        <SectionBoundary name="settings"><Suspense fallback={<ModalLoadingFallback />}>
          <SettingsModal
            ...
            onStatusChange={(status: string) => {
              ...
              localStorage.setItem('d_status', status);
              setPresenceMap(p => ({ ...p, [api.userId!]: status }));
              if (status === 'online') { setManualStatus(null); localStorage.removeItem('d_manual_status'); }
              else { setManualStatus(status); localStorage.setItem('d_manual_status', status); }
              api.ws?.send(JSON.stringify({ type: 'status_change', status }));
          ...
          <div style={{ padding: 20 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 16 }}>
              {([
                ...
                <div key={t.id} onClick={() => setCreateChannelType(t.id)} style={{ padding: '8px 6px', borderRadius: 'var(--border-radius)', cursor: 'pointer', border: `2px solid ${createChannelType === t.id ? T.ac : T.bd}`, textAlign: 'center', fontSize: 12, fontWeight: 600, color: createChannelType === t.id ? T.ac : T.mt, background: createChannelType === t.id ? `${ta(T.ac,'11')}` : 'transparent' }}>
      ...
      {modal === 'invite-config' && curServer && (
        <Modal title={`Invite people to ${curServer.name}`} onClose={() => { setModal(null); setInviteResult(''); }}>
          <div style={{ padding: 20, minWidth: 340 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 20, padding: '12px 14px', background: T.sf2, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}` }}>
            ...
            {inviteResult ? (
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  <div style={{ flex: 1, padding: '10px 12px', background: T.bg, border: `1px solid ${T.bd}`, borderRadius: 6, fontSize: 13, fontFamily: 'var(--font-mono)', color: T.ac, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            ...
            {inviteResult && !showInviteQr && (
              <button onClick={() => setShowInviteQr(true)} style={{ width: '100%', marginTop: 10, padding: '9px 0', fontSize: 12, fontWeight: 600, borderRadius: 'var(--radius-md)', cursor: 'pointer', background: T.sf2, color: T.tx, border: `1px solid ${T.bd}` }}>
                Share Offline (QR Code)
              </button>
            )}
            ...
            {inviteResult && (
              <button onClick={() => setShowServerQrConnect(true)} style={{ width: '100%', marginTop: 10, padding: '9px 0', fontSize: 12, fontWeight: 600, borderRadius: 'var(--radius-md)', cursor: 'pointer', background: `linear-gradient(135deg,${ta(T.ac,'22')},${ta(T.ac,'22')})`, color: T.ac, border: `1px solid ${ta(T.ac,'44')}` }}>
                Shareable QR Code (24h)
              </button>
            )}
            {showServerQrConnect && curServer && (
              <Suspense fallback={null}><QrConnectModal type="server" serverId={curServer.id} onClose={() => setShowServerQrConnect(false)} /></Suspense>
            )}
          </div>
        </Modal>
      )}
        ...
        <Modal title="Set Status" onClose={() => setModal(null)}>
          <div style={{ padding: 16 }}>
            {[
              ...
              <div key={s.key} onClick={() => changeStatus(s.key)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 'var(--border-radius)', cursor: 'pointer', marginBottom: 4, background: userStatus === s.key ? 'rgba(124,58,237,0.08)' : 'transparent', border: userStatus === s.key ? `1px solid ${ta(T.ac,'33')}` : '1px solid transparent' }}
                onMouseEnter={e => e.currentTarget.style.background = 'rgba(255,255,255,0.03)'}
            ...
            <div style={{ flex: 1, padding: 20, overflowY: 'auto' }}>
              {chSettingsTab === 'overview' && (<>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                    <div onClick={() => setChNsfw(!chNsfw)} style={{ width: 36, height: 20, borderRadius: 10, background: chNsfw ? T.ac : T.sf2, position: 'relative', cursor: 'pointer', transition: 'background .2s' }}><div style={{ width: 16, height: 16, borderRadius: 'var(--border-radius)', background: '#fff', position: 'absolute', top: 2, left: chNsfw ? 18 : 2, transition: 'left .2s' }} /></div>
                    <span style={{ fontSize: 13 }}>NSFW Channel</span>
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                    <div onClick={() => setChArchived(!chArchived)} style={{ width: 36, height: 20, borderRadius: 10, background: chArchived ? T.ac : T.sf2, position: 'relative', cursor: 'pointer', transition: 'background .2s' }}><div style={{ width: 16, height: 16, borderRadius: 'var(--border-radius)', background: '#fff', position: 'absolute', top: 2, left: chArchived ? 18 : 2, transition: 'left .2s' }} /></div>
                        ...
                        {CH_PERMS.map(({ key: permKey, label, desc }) => {
                          ...
                          return (
                            <div key={permKey} style={{ display: 'flex', alignItems: 'center', padding: '9px 16px', gap: 12, borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                              ...
                              <div style={{ display: 'flex', borderRadius: 'var(--border-radius)', overflow: 'hidden', border: `1px solid ${T.bd}`, flexShrink: 0 }}>
              ...
              {chSettingsTab === 'danger-zone' && (<>
                ...
                <div style={{ padding: 16, background: 'rgba(255,71,87,0.05)', border: '1px solid rgba(255,71,87,0.2)', borderRadius: 10 }}>
                  ...
                  }} style={{ padding: '8px 16px', background: 'rgba(255,71,87,0.15)', border: '1px solid rgba(255,71,87,0.4)', borderRadius: 'var(--border-radius)', color: T.err, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Delete Channel</button>
                </div>
              </>)}
            </div>
          </div>
      ...
      {showGifPicker && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999 }} onClick={() => setShowGifPicker(false)}>
          <div style={{ position: 'fixed', bottom: 70, right: panel ? 240 : 20, zIndex: 10000 }} onClick={e => e.stopPropagation()}>
            <Suspense fallback={InlineLoader}><GifPicker onSelect={async (url) => {
            ...
            }} onClose={() => setShowGifPicker(false)} /></Suspense>
          </div>
        </div>
      )}
      ...
      {showAddPeople && voiceChannel && (
        <Suspense fallback={InlineLoader}><AddPeopleModal
      ...
      {voiceConfirm && (
        <Suspense fallback={null}><VoiceConfirmationModal
      ...
      {showScheduleModal && curChannel && (
        <Suspense fallback={null}><ScheduleModal
          channelId={curChannel.id}
          channelName={curChannel.name}
      ...
      {view === 'server' && !openThread && panel === 'search' && (
          <Suspense fallback={InlineLoader}><SearchPanel
            messages={messages}
            dmMsgs={dmMsgs}
            members={members}
            channels={channels}
          ...
          <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 350, background: T.sf, borderLeft: `1px solid ${T.bd}`, zIndex: 9999, display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 24px rgba(0,0,0,0.4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: `1px solid ${T.bd}` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 700, fontSize: 15, color: T.tx }}>
                ...
                {total > 0 && <span style={{ fontSize: 11, fontWeight: 600, color: T.mt, background: T.sf2, padding: '1px 6px', borderRadius: 'var(--radius-md)' }}>{total}/50</span>}
              </div>
      ...
      {quickSwitcher && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '15vh', zIndex: 10002 }} onClick={() => setQuickSwitcher(false)}>
          <div onClick={e => e.stopPropagation()} style={{ width: 520, maxWidth: '90vw', background: T.sf, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}`, overflow: 'hidden', boxShadow: '0 16px 48px rgba(0,0,0,0.5)' }}>
            <input autoFocus value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Where would you like to go?" style={{ width: '100%', padding: '16px 20px', background: 'transparent', border: 'none', color: T.tx, fontSize: 16, outline: 'none', fontFamily: 'var(--font-primary)' }} onKeyDown={e => { if (e.key === 'Escape') setQuickSwitcher(false); }} />
            ...
            <div onClick={e => e.stopPropagation()} style={{ width: 400, maxWidth: '90vw', background: T.sf, borderRadius: 14, border: `1px solid ${T.bd}`, padding: 24, boxShadow: '0 16px 48px rgba(0,0,0,0.5)' }}>
              {submitted ? (
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                  ...
                  <button onClick={() => setReportTarget(null)} style={{ padding: '6px 20px', borderRadius: 'var(--radius-md)', border: 'none', background: T.ac, color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>Close</button>
                </div>
              ) : (<>
                ...
                <div style={{ padding: '8px 12px', background: T.bg, borderRadius: 'var(--radius-md)', border: `1px solid ${T.bd}`, marginBottom: 12, fontSize: 12, color: T.mt, lineHeight: 1.5, maxHeight: 60, overflow: 'hidden' }}>
                ...
                <select value={reason} onChange={e => setReason(e.target.value)} style={{ width: '100%', padding: '8px 10px', background: T.bg, border: `1px solid ${T.bd}`, borderRadius: 'var(--radius-md)', color: T.tx, fontSize: 13, marginBottom: 10 }}>
                ...
                <textarea value={details} onChange={e => setDetails(e.target.value)} placeholder="Additional context..." rows={3} style={{ width: '100%', padding: '8px 10px', background: T.bg, border: `1px solid ${T.bd}`, borderRadius: 'var(--radius-md)', color: T.tx, fontSize: 12, resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: 12 }} />
                <button onClick={async () => {
                  setSubmitting(true);
                  try { await api.submitReport(reportTarget.id, reason, details.trim() || undefined); setSubmitted(true); } catch {}
                  setSubmitting(false);
                }} disabled={submitting} style={{ padding: '8px 24px', borderRadius: 'var(--radius-md)', border: 'none', background: submitting ? T.sf2 : T.err, color: '#fff', fontSize: 13, fontWeight: 700, cursor: submitting ? 'default' : 'pointer' }}>
          ...
          <div onClick={e => e.stopPropagation()} style={{ width: 400, maxWidth: '90vw', background: T.sf, borderRadius: 14, border: `1px solid ${T.bd}`, padding: 28, textAlign: 'center', boxShadow: '0 16px 48px rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              {tierLimitModal.tier !== 'enterprise' && (
                <button onClick={() => { setTierLimitModal(null); setModal('upgrade'); }} style={{ padding: '10px 28px', borderRadius: 10, border: 'none', background: `linear-gradient(135deg,${T.ac},${T.ac2})`, color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Upgrade</button>
              )}
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 15000 }} onClick={() => setStreamSetupModal(null)}>
          <div onClick={e => e.stopPropagation()} style={{ width: 480, maxWidth: '90vw', background: T.sf, borderRadius: 14, border: `1px solid ${T.bd}`, padding: 28, boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <div onClick={() => setStreamSetupModal(null)} style={{ flex: 1, padding: '10px 0', textAlign: 'center', borderRadius: 'var(--border-radius)', cursor: 'pointer', fontSize: 13, fontWeight: 600, background: T.sf2, color: T.tx, border: `1px solid ${T.bd}` }}>Done</div>
              <div onClick={stopGoLive} style={{ flex: 1, padding: '10px 0', textAlign: 'center', borderRadius: 'var(--border-radius)', cursor: 'pointer', fontSize: 13, fontWeight: 700, background: 'rgba(255,71,87,0.15)', color: T.err, border: '1px solid rgba(255,71,87,0.35)' }}>Stop Streaming</div>
            </div>
          </div>
        </div>
      )}
      ...
      {/* Toast */}
      {toast && (
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', background: toast.startsWith('Send failed') ? T.err : T.ac, color: '#fff', padding: '10px 24px', borderRadius: 'var(--border-radius)', fontSize: 13, fontWeight: 600, zIndex: 20000, boxShadow: '0 4px 16px rgba(0,0,0,0.4)' }}>{toast}</div>
      )}
      ...
      {showOnboarding && (
        <Suspense fallback={null}><OnboardingModal username={api.username || ''} onThemeChange={handleThemeChange} onLayoutChange={(id) => {
          localStorage.setItem('d_layout_mode', id);
          if (id === 'simple' || id === 'standard' || id === 'power') layoutMode.setMode(id);
          ...
          localStorage.setItem('onboarding_complete', 'true');
          api.updateProfile({ onboarding_complete: true }).catch(() => {});
      ...
      <BugReportButton showToast={(msg) => { setToast(msg); setTimeout(() => setToast(''), 4000); }} externalOpen={showBugReport} onExternalClose={() => setShowBugReport(false)} />

      {/* Bottom tab bar (Simple layout + mobile) */}
      {isCompact && <MobileBottomTabs activeTab={mobileTab} onTabChange={handleMobileTab} unreadCount={totalDmUnread} />}
    </div>
  );
}
...
function DeleteServerModal({ serverId, serverName, onTransferInstead, onDeleted, onClose }: {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 10002, padding: 16 }}
      ...
      <div style={{ width: '100%', maxWidth: 440, background: T.sf, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}`, boxShadow: '0 12px 48px rgba(0,0,0,0.5)', padding: 24 }}>
        ...
        {step === 'ask' && (<>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <button onClick={onClose} style={{ padding: '8px 18px', borderRadius: 'var(--radius-md)', border: 'none', background: 'none', color: T.mt, fontSize: 12, cursor: 'pointer' }}>Cancel</button>
          </div>
        </>)}

        {step === 'confirm' && (<>
          {/* Danger warning */}
          <div style={{ padding: '14px 16px', background: 'rgba(255,71,87,0.1)', border: '1px solid rgba(255,71,87,0.3)', borderRadius: 'var(--radius-md)', marginBottom: 16, lineHeight: 1.6 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => { setStep('ask'); setConfirmName(''); setPassword(''); setError(''); }}
              style={{ padding: '8px 18px', borderRadius: 'var(--radius-md)', border: `1px solid ${T.bd}`, background: T.sf2, color: T.mt, fontSize: 12, cursor: 'pointer' }}>Back</button>
