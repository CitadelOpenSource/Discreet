...
export function InvitePreview({ url, joinedServerIds, onJoined }: InvitePreviewProps) {
  ...
  if (state === 'loading') {
    return (
      <div style={{ ...card, opacity: 0.5 }}>
        <div style={{ width: 40, height: 40, borderRadius: 'var(--border-radius)', background: T.bd }} />
        <div style={{ flex: 1 }}>
  ...
  if (state === 'expired') {
    return (
      <div style={{ ...card, opacity: 0.6 }}>
        <div style={{ width: 40, height: 40, borderRadius: 'var(--border-radius)', background: T.bd, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: T.mt }}>?</div>
        <div style={{ flex: 1 }}>
  ...
  if (state === 'external') {
    return (
      <div style={card}>
        <div style={{ width: 40, height: 40, borderRadius: 'var(--border-radius)', background: `${ta(T.warn,'18')}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>🌐</div>
        ...
        <button
          ...
          style={{ padding: '6px 14px', fontSize: 11, fontWeight: 700, borderRadius: 'var(--radius-md)', cursor: 'pointer', background: T.sf3, color: T.tx, border: `1px solid ${T.bd}`, whiteSpace: 'nowrap', flexShrink: 0 }}
        >
          Open
        </button>
      </div>
  ...
  return (
    <div style={card}>
      {/* Server icon */}
      <div style={{ width: 40, height: 40, borderRadius: 'var(--border-radius)', background: info?.icon_url ? 'transparent' : `linear-gradient(135deg,${ta(T.ac,'33')},${ta(T.ac2,'33')})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 700, color: T.ac, overflow: 'hidden', flexShrink: 0 }}>
        {info?.icon_url
      ...
      {/* Action button */}
      {isJoined ? (
        <span style={{ padding: '6px 14px', fontSize: 11, fontWeight: 600, borderRadius: 'var(--radius-md)', background: T.sf3, color: T.mt, border: `1px solid ${T.bd}`, whiteSpace: 'nowrap', flexShrink: 0 }}>
          Already Joined
        </span>
      ) : (
        <button
          onClick={handleJoin}
          disabled={joining}
          style={{ padding: '6px 14px', fontSize: 11, fontWeight: 700, borderRadius: 'var(--radius-md)', cursor: joining ? 'default' : 'pointer', background: T.ac, color: '#000', border: 'none', whiteSpace: 'nowrap', flexShrink: 0, opacity: joining ? 0.6 : 1 }}
        >
