  ...
  return (
    <div style={{ background: T.sf, border: `1px solid ${T.bd}`, borderRadius: 14, padding: 16 }}>
      <div style={{ display: 'flex', gap: 12, marginBottom: 10 }}>
        <div style={{ ...pulse, width: 48, height: 48, borderRadius: 12, flexShrink: 0 }} />
        <div style={{ flex: 1 }}>
...
export function DiscoverPanel({ onJoin }: DiscoverPanelProps) {
  ...
  const fetchServers = useCallback(async (q: string, cat: Category, reset: boolean) => {
    setLoading(true);
    setError('');
    try {
      const raw: DiscoveredServer[] = await api.discoverServers(q || undefined, cat);
      ...
      if (reset) {
        ...
        (fetchServers as any)._full = list;
      }
    } catch {
  ...
  // Initial load + on category change (immediate)
  useEffect(() => {
    void fetchServers(query, category, true);
  ...
  const loadMore = async () => {
    const full: DiscoveredServer[] = (fetchServers as any)._full || [];
        ...
        <div style={{
          ...
          <input
            ...
            style={{
              ...
              color: T.tx, fontSize: 14, fontFamily: "'DM Sans', sans-serif",
            }}
          />
          {query && (
            <button
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 24px 24px' }}>
        ...
        {error && (
          <div style={{
            ...
            <button
              onClick={() => fetchServers(query, category, true)}
              style={{
                padding: '8px 20px', borderRadius: 8, border: `1px solid ${T.bd}`,
