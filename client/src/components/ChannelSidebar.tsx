...
function chIcon(ch: Channel) {
  if (ch.is_archived)                     return <span style={{ opacity: 0.5 }}><I.Archive s={14} /></span>;
  if (ch.channel_type === 'voice')        return <I.Vol />;
  if (ch.channel_type === 'forum')        return <I.Msg />;
  if (ch.channel_type === 'announcement') return <I.Megaphone s={14} />;
  if (ch.channel_type === 'stage')        return <I.Mic s={14} />;
  if (ch.read_only) return <I.Megaphone s={14} />;
...
export function ChannelSidebar({
  ...
  unreadCounts, mentionCounts, mutedChannels, videoStreams, streamStatus,
  ...
  const activeChannels = safeChannels.filter(c => !c.is_archived);
  const archivedChannels = safeChannels.filter(c => c.is_archived).filter(canSeeChannel);
  ...
  return (
    <div data-testid="channel-list" data-component="ChannelSidebar" style={{ flex: 1, overflowY: 'auto', padding: '8px 6px' }}>

      {/* Empty state when no channels exist */}
      {safeChannels.length === 0 && (
        <div style={{ padding: '24px 12px', textAlign: 'center', color: T.mt }}>
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>No channels yet</div>
          <div style={{ fontSize: 11 }}>Create one to get started.</div>
        </div>
      )}
