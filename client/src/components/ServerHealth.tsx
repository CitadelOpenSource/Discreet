  ...
  return (
    <div style={{ background: T.sf2, borderRadius: 10, padding: '14px 16px', border: `1px solid ${loading ? T.bd : status === 'healthy' ? dot + '33' : status === 'unknown' ? T.bd : dot + '44'}`, position: 'relative', overflow: 'hidden', transition: 'border-color .3s' }}>
      ...
      <div style={{ paddingLeft: 8 }}>
        ...
        {/* value */}
        <div style={{ fontSize: 22, fontWeight: 800, color: loading ? T.mt : (status === 'unknown' ? T.tx : dot), lineHeight: 1, marginBottom: 4, fontFamily: 'var(--font-mono)', transition: 'color .3s' }}>
          {loading ? '…' : value}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {latency !== undefined && !loading && (
            <span style={{ fontSize: 10, color: latency < 50 ? '#3ba55d' : latency < 150 ? '#f0b232' : '#ff4757', fontFamily: 'var(--font-mono)' }}>
              {latency}ms
            </span>
          )}
          {sub && !loading && (
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px 40px', minWidth: 0 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          >
            <div style={{ position: 'absolute', top: 2, left: autoRefresh ? 18 : 2, width: 16, height: 16, borderRadius: 'var(--radius-md)', background: '#fff', transition: 'left .2s', boxShadow: '0 1px 3px rgba(0,0,0,0.3)' }} />
          </div>
          {autoRefresh && (
            <span style={{ fontSize: 11, color: T.mt, fontFamily: 'var(--font-mono)', minWidth: 20, textAlign: 'right' }}>{countdown}s</span>
          )}
        </div>
        ...
        <button
          ...
          disabled={loading}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', background: T.sf2, border: `1px solid ${T.bd}`, borderRadius: 'var(--radius-md)', color: loading ? T.mt : T.tx, fontSize: 12, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', transition: 'border-color .15s' }}
      ...
      {/* ── error banner ── */}
      {error && (
        <div style={{ padding: '10px 14px', background: '#ff475712', border: '1px solid #ff475733', borderRadius: 'var(--radius-md)', marginBottom: 16, fontSize: 12, color: '#ff4757' }}>
      ...
      {/* ── latency sparkline ── */}
      <div style={{ background: T.sf2, borderRadius: 'var(--border-radius)', padding: '18px 20px', border: `1px solid ${T.bd}`, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          ...
          <div style={{ textAlign: 'right' }}>
            {latestPing !== undefined && (
              <>
                <div style={{ fontSize: 22, fontWeight: 800, color: latestPing < 50 ? '#3ba55d' : latestPing < 150 ? '#f0b232' : '#ff4757', fontFamily: 'var(--font-mono)' }}>
                  {latestPing}ms
                </div>
          ...
          <div style={{ display: 'flex', gap: 20, marginTop: 10, flexWrap: 'wrap' }}>
            {[
              ...
              <div key={label}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: T.tx, fontFamily: 'var(--font-mono)' }}>{value}ms</div>
              </div>
            ))}
          </div>
        )}
