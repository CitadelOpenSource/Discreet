    ...
    <div style={{ width: 320, minWidth: 320, borderLeft: `1px solid ${T.bd}`, background: T.sf, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '10px 12px 6px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, ...getInp(), padding: '8px 12px' }}>
          ...
          <input ref={inputRef} value={query} onChange={e => setQuery(e.target.value)}
            ...
            style={{ flex: 1, background: 'transparent', border: 'none', color: T.tx, fontSize: 13, outline: 'none', fontFamily: 'var(--font-primary)' }} />
      ...
      {!query && tab === 'messages' && (
        <div style={{ padding: '8px 14px', fontSize: 11, color: T.mt, lineHeight: 1.6 }}>
          ...
          <div><span style={{ color: T.ac, fontFamily: 'var(--font-mono)' }}>from:username</span> — filter by author</div>
          <div><span style={{ color: T.ac, fontFamily: 'var(--font-mono)' }}>in:#channel</span> — filter by channel</div>
          <div><span style={{ color: T.ac, fontFamily: 'var(--font-mono)' }}>has:link</span> — messages containing URLs</div>
          <div><span style={{ color: T.ac, fontFamily: 'var(--font-mono)' }}>has:file</span> — messages with attachments</div>
          <div><span style={{ color: T.ac, fontFamily: 'var(--font-mono)' }}>is:pinned</span> — pinned messages only</div>
          <div><span style={{ color: T.ac, fontFamily: 'var(--font-mono)' }}>before:2026-03-01</span> — before date</div>
          <div><span style={{ color: T.ac, fontFamily: 'var(--font-mono)' }}>after:2026-01-15</span> — after date</div>
          <div style={{ marginTop: 4, color: T.mt, fontStyle: 'italic' }}>Combine: <span style={{ color: T.ac }}>from:john has:link is:pinned</span></div>
          <div style={{ marginTop: 8, padding: '10px 12px', background: 'rgba(0,212,170,0.05)', borderRadius: 'var(--border-radius)', border: '1px solid rgba(0,212,170,0.15)', fontSize: 11, display: 'flex', alignItems: 'flex-start', gap: 8 }}>
          ...
          {results.map(m => {
            ...
            return (
              <div key={m.id} onClick={() => { const target: NavigateTarget = { type: 'channel', messageId: m.id }; if (ch) target.channel = ch; else if (curChannel) target.channel = curChannel; if (onNavigate && target.channel) onNavigate(target); }}
                style={{ padding: '8px 10px', margin: '2px 0', borderRadius: 'var(--border-radius)', cursor: 'pointer', background: T.sf2, border: `1px solid ${T.bd}`, transition: 'background .15s' }}
          ...
          {memberResults.map(m => {
            ...
            return (
              <div key={uid} onClick={() => onNavigate?.({ type: 'user', userId: uid })}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', margin: '2px 0', borderRadius: 'var(--border-radius)', cursor: 'pointer', background: T.sf2, border: `1px solid ${T.bd}` }}
                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(0,212,170,0.04)')}
                onMouseLeave={e => (e.currentTarget.style.background = T.sf2)}>
          ...
          {userResults.map(u => {
            ...
            return (
              <div key={u.id} onClick={() => onNavigate?.({ type: 'user', userId: u.id })}
                style={{ padding: '10px 12px', margin: '2px 0', borderRadius: 'var(--border-radius)', cursor: 'pointer', background: T.sf2, border: `1px solid ${T.bd}` }}
                onMouseEnter={e => (e.currentTarget.style.background = 'rgba(0,212,170,0.04)')}
                onMouseLeave={e => (e.currentTarget.style.background = T.sf2)}>
