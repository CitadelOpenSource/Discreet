...
function parseInline(text: string, ctx: InlineCtx, keyBase = 0): React.ReactNode[] {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    if (tok.startsWith('**') && tok.endsWith('**') && tok.length > 4) {
      ...
      parts.push(
        <code key={k} style={{ background: T.bg, padding: '1px 5px', borderRadius: 3, fontSize: '0.9em', fontFamily: "'Fira Code','Cascadia Code',monospace" }}>
          {tok.slice(1, -1)}
        </code>
      );
    ...
    if (seg.type === 'codeblock') {
      ...
      elements.push(
        <div key={key} style={{ position: 'relative', margin: '4px 0' }}>
          {seg.lang && (
            <div style={{ position: 'absolute', top: 4, right: 8, fontSize: 10, color: T.mt, fontFamily: "'Fira Code','Cascadia Code',monospace", textTransform: 'uppercase', letterSpacing: '0.5px', opacity: 0.7 }}>
              {seg.lang}
            </div>
          )}
          <pre style={{ background: T.bg, border: `1px solid ${T.bd}`, borderRadius: 6, padding: '10px 12px', margin: 0, overflowX: 'auto', fontSize: 13, lineHeight: 1.5, fontFamily: "'Fira Code','Cascadia Code',monospace", color: T.tx, whiteSpace: 'pre-wrap' }}>
