...
export function DocumentEditor({ channelId, onClose }: DocumentEditorProps) {
  ...
  const saveDoc = useCallback(async (id: string, docTitle: string) => {
    ...
    try {
      ...
      void e; // save failed — user sees stale state as implicit indicator
    } finally {
      setSaving(false);
    }
  }, [channelId]);
          ...
          <>
            ...
            {shareUrl && (
              <div style={{ padding: '8px 16px', background: `${ta(T.ac,'12')}`, borderBottom: `1px solid ${ta(T.ac,'30')}`, display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                ...
                <code style={{ flex: 1, fontSize: 10, color: T.mt, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: 'var(--font-mono)' }}>
                  {shareUrl}
                </code>
