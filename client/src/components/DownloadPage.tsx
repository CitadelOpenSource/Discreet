...
function useS() {
  ...
  return {
    ...
    subtitle: { fontSize: 14, color: C.mt, marginBottom: 40 } as React.CSSProperties,
    ...
    card: (available: boolean) => ({
      padding: '20px', background: C.sf2, borderRadius: 'var(--border-radius)', border: `1px solid ${C.bd}`,
      opacity: available ? 1 : 0.55, position: 'relative' as const,
    }),
    icon: { fontSize: 28, marginBottom: 10, display: 'block' } as React.CSSProperties,
    name: { fontSize: 16, fontWeight: 700, color: C.tx, marginBottom: 4 } as React.CSSProperties,
    desc: { fontSize: 12, color: C.mt, lineHeight: 1.5, marginBottom: 14 } as React.CSSProperties,
    btn: (primary: boolean) => ({
      display: 'inline-block', padding: '8px 20px', borderRadius: 'var(--radius-md)', border: 'none',
      background: primary ? C.ac : C.sf2, color: primary ? '#000' : C.mt,
    ...
    hash: { fontSize: 10, fontFamily: 'monospace', color: C.mt, marginTop: 8, wordBreak: 'break-all' as const } as React.CSSProperties,
    section: { marginBottom: 32 } as React.CSSProperties,
    h2: { fontSize: 20, fontWeight: 700, color: C.ac, marginBottom: 12 } as React.CSSProperties,
    code: { display: 'block', padding: '12px 14px', background: C.bg, borderRadius: 'var(--radius-md)', border: `1px solid ${C.bd}`, fontFamily: 'var(--font-mono)', fontSize: 12, color: C.ac, overflowX: 'auto' as const, marginBottom: 8 } as React.CSSProperties,
    ...
    <div style={S.page}>
      <div style={S.wrap}>
            ...
            <div key={p.id} style={S.card(p.available)}>
              ...
              {'virusTotal' in p && p.virusTotal && (
                <div style={{ marginTop: 4 }}>
                  <a href={p.virusTotal} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10, color: S.ac }}>View on VirusTotal</a>
                </div>
              )}
              {'note' in p && p.note && (
        ...
        <div style={S.section}>
          ...
          <p style={{ fontSize: 13, color: S.mt, lineHeight: 1.6, marginBottom: 16 }}>
            After downloading, verify the file integrity by comparing its SHA-256 hash against the one published on the{' '}
            <a href="https://github.com/CitadelOpenSource/Discreet/releases" style={{ color: S.ac }}>GitHub release page</a>.
          </p>

          <div style={{ fontSize: 11, fontWeight: 700, color: S.mt, textTransform: 'uppercase', marginBottom: 6 }}>Windows (PowerShell)</div>
          <code style={S.code}>Get-FileHash .\Discreet-Setup.msi -Algorithm SHA256 | Format-List</code>

          <div style={{ fontSize: 11, fontWeight: 700, color: S.mt, textTransform: 'uppercase', marginBottom: 6, marginTop: 12 }}>macOS / Linux</div>
          <code style={S.code}>shasum -a 256 Discreet.dmg</code>

          <p style={{ fontSize: 12, color: S.mt, marginTop: 12, lineHeight: 1.6 }}>
        ...
        <div style={S.section}>
          ...
          <p style={{ fontSize: 13, color: S.mt, lineHeight: 1.6 }}>
            If you encounter issues with installation or the application, use the bug report button (bottom-left corner in the app) or{' '}
            <a href="https://github.com/CitadelOpenSource/Discreet/issues" style={{ color: S.ac }}>open an issue on GitHub</a>.
          </p>
        </div>
