...
export function BadgeDisplay({ tier, isBot, isFounder, isSelf, isAdminContext, platformRole }: Props) {
  ...
  if (isFounder) {
    badges.push(
      <span key="founder" title="Founder" style={{ display: 'inline-flex', color: '#f0b232', marginInlineStart: 3 }}>
  ...
  if (isBot) {
    badges.push(
      <span key="bot" style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: '#5865f222', color: '#7984f5', fontWeight: 700, marginInlineStart: 4, letterSpacing: '0.3px', verticalAlign: 'middle' }}>BOT</span>
    );
  }
  ...
  if (tier === 'pro') {
    badges.push(
      <span key="pro" title="Pro" style={{ display: 'inline-flex', color: '#f0b232', marginInlineStart: 3 }}>
  ...
  if (tier === 'team') {
    badges.push(
      <span key="team" title="Team" style={{ display: 'inline-flex', color: '#3b82f6', marginInlineStart: 3 }}>
  ...
  if (isAdminContext && (platformRole === 'admin' || platformRole === 'dev')) {
    badges.push(
      <span key="admin" title="Admin" style={{ display: 'inline-flex', color: T.ac, marginInlineStart: 3 }}>
  ...
  if (isSelf && (tier === 'verified' || tier === 'pro' || tier === 'team' || tier === 'admin')) {
    badges.push(
      <span key="verified" title="Email verified" style={{ display: 'inline-flex', color: T.ac, marginInlineStart: 3 }}>
