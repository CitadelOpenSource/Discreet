...
export function EmojiPicker({ onSelect, onClose, full, customEmoji = [] }: EmojiPickerProps) {
  ...
  return (
    <div ref={ref} style={{ position: 'absolute', bottom: '100%', right: 0, marginBottom: 4, width: 352, maxHeight: 420, background: T.sf, borderRadius: 'var(--border-radius)', border: `1px solid ${T.bd}`, zIndex: 100, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {/* Search */}
      <div style={{ padding: '8px 10px 4px' }}>
        <input ref={inputRef} value={search} onChange={e => setSearch(e.target.value)} placeholder="Search emojis..." style={{ width: '100%', background: T.bg, border: `1px solid ${T.bd}`, borderRadius: 6, color: T.tx, fontSize: 12, padding: '6px 10px', outline: 'none', boxSizing: 'border-box', fontFamily: 'var(--font-primary)' }} />
      </div>
