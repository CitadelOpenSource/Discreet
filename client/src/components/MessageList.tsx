...
export function MessageList(props: MessageListProps) {
  ...
  const {
    ...
    msgDensity, chatFontSize, showLinkPreviews, showTimestamps,
  ...
  return (
    ...
    >
      ...
      {visibleMessages.map((m, vi) => {
        ...
        // Message grouping: same author within 5 min, no date separator, not a system message
        const isGrouped = !showDateSep
          && prevMsg
          && prevMsg.author_id === m.author_id
          && !prevMsg.is_auto_reply
          && !m.is_auto_reply
          ...
          <React.Fragment key={m.id}>
            ...
            <div
              ...
              style={{
                ...
                padding: isGrouped
                  ? (msgDensity === 'compact' ? '0px 16px' : '1px 16px')
                  : (msgDensity === 'compact' ? '1px 16px' : msgDensity === 'cozy' ? '6px 16px' : '4px 16px'),
                position: 'relative',
              ...
              onMouseEnter={e => {
                ...
                const ts = e.currentTarget.querySelector('.msg-grouped-ts') as HTMLElement;
                if (ts) ts.style.opacity = '1';
              }}
              onMouseLeave={e => {
                if (highlightedMsg !== m.id) e.currentTarget.style.background = 'transparent';
                const ts = e.currentTarget.querySelector('.msg-grouped-ts') as HTMLElement;
            ...
            >
              ...
              {isGrouped ? (
                <div className="msg-avatar-spacer" style={{ width: msgDensity === 'compact' ? 28 : msgDensity === 'cozy' ? 44 : 36, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {/* Hover-revealed compact timestamp */}
                  <span className="msg-grouped-ts" title={tzCtx.formatFullTooltip(m.created_at)}
                    style={{ fontSize: 9, color: T.mt, opacity: 0, whiteSpace: 'nowrap', cursor: 'default', transition: 'opacity 0.15s' }}>
                    {tzCtx.formatTime(m.created_at, { hour: 'numeric', minute: '2-digit' })}
                  </span>
                </div>
              ) : (
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  ...
                  {showTimestamps && msgDensity === 'compact' && <span title={tzCtx.formatFullTooltip(m.created_at)} style={{ fontSize: 9, color: T.mt, cursor: 'default', whiteSpace: 'nowrap' }}>{tzCtx.formatTimestamp(m.created_at)}</span>}
                  <span style={{ flex: 1 }} />
                  {showTimestamps && msgDensity !== 'compact' && <span title={tzCtx.formatFullTooltip(m.created_at)} style={{ fontSize: 10, color: T.mt, cursor: 'default', whiteSpace: 'nowrap' }}>{tzCtx.formatTimestamp(m.created_at)}</span>}
                ...
                m.is_translation ? (
                  <div style={{ marginTop: 4, paddingLeft: 10, borderLeft: `2px solid ${T.ac}`, marginLeft: 2 }}>
                    ...
                    <div className="msg-text" style={{ fontSize: chatFontSize, lineHeight: msgDensity === 'compact' ? 1.3 : msgDensity === 'cozy' ? 1.6 : 1.5, wordBreak: 'break-word', opacity: failedMessages[m.id] ? 0.5 : 1 }}>
                      {m.renderModel ? <KernelContent content={m.renderModel.content} onMention={onMention} mentionStyle={mentionStyle} /> : <Markdown text={msgText} onMention={onMention} mentionStyle={mentionStyle} />}
                    </div>
                  </div>
                ...
                /* Message text — use kernel render model when available, fall back to Markdown */
                <div className="msg-text" style={{ fontSize: chatFontSize, lineHeight: msgDensity === 'compact' ? 1.3 : msgDensity === 'cozy' ? 1.6 : 1.5, wordBreak: 'break-word', opacity: failedMessages[m.id] ? 0.5 : 1 }}>
                  {m.renderModel ? <KernelContent content={m.renderModel.content} onMention={onMention} mentionStyle={mentionStyle} /> : <Markdown text={msgText} onMention={onMention} mentionStyle={mentionStyle} />}
                </div>
                )}
                ...
                {/* Attachments */}
                {(m as any).attachments?.map((a: any, aIdx: number) => {
                  const allAttach: MediaItem[] = ((m as any).attachments || []).map((att: any) => ({
                    url: att.url, thumbnailUrl: att.thumbnail_url, filename: att.filename || 'file',
                    contentType: att.content_type || 'application/octet-stream',
                    size: att.size, uploader: getName(m.author_id),
                  }));
                  return (
                    <div key={a.id || a.url} style={{ marginTop: 4 }}>
                      {a.content_type?.startsWith('image/') ? (
                        <img src={a.thumbnail_url || a.url} alt={a.filename}
                          onClick={(e) => { e.stopPropagation(); setMediaViewer({ items: allAttach, index: aIdx }); }}
                          style={{ maxWidth: 300, maxHeight: 200, borderRadius: 'var(--radius-md)', cursor: 'pointer', transition: 'opacity 0.15s' }}
                          onMouseEnter={e => (e.currentTarget.style.opacity = '0.85')}
                          onMouseLeave={e => (e.currentTarget.style.opacity = '1')}
                        />
                      ) : a.content_type?.startsWith('video/') ? (
                        <div onClick={(e) => { e.stopPropagation(); setMediaViewer({ items: allAttach, index: aIdx }); }}
                          style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: T.sf2, borderRadius: 'var(--radius-md)', border: `1px solid ${T.bd}`, cursor: 'pointer' }}>
                          <I.Camera s={16} />
                          <span style={{ fontSize: 12, color: T.tx, fontWeight: 500 }}>{a.filename || 'Video'}</span>
                          {a.size && <span style={{ fontSize: 11, color: T.mt }}>({(a.size / 1024 / 1024).toFixed(1)} MB)</span>}
                        </div>
                      ) : (
                        <div onClick={(e) => { e.stopPropagation(); setMediaViewer({ items: allAttach, index: aIdx }); }}
                          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 10px', background: T.sf2, borderRadius: 6, border: `1px solid ${T.bd}`, color: T.ac, fontSize: 12, cursor: 'pointer' }}>
                ...
                {(m.reply_count ?? 0) > 0 && (
                  <div onClick={() => onOpenThread(m)}
                    style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 4, padding: '2px 8px', background: `${ta(T.ac,'14')}`, borderRadius: 'var(--radius-md)', cursor: 'pointer', fontSize: 11, color: T.ac, fontWeight: 600, border: `1px solid ${ta(T.ac,'22')}`, transition: 'background .15s' }}
