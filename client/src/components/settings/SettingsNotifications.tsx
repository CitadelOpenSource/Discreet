...
export default function SettingsNotifications({
  ...
  return (<>
    ...
    <NRow label="Mentions-only mode" sub="Only show a badge/alert when you are directly @mentioned" on={ns.mentionsOnly} onToggle={() => { setN('mentionsOnly', !ns.mentionsOnly, 'd_notif_mentions_only'); save('mentions_only', !ns.mentionsOnly); }} />
    <NRow label="Group notifications" sub="Bundle multiple messages from the same channel into one alert" on={ns.group} onToggle={() => { setN('group', !ns.group, 'd_notif_group'); save('group_notifications', !ns.group); }} />
    ...
    <NRow label="Enable sounds" sub="Master switch for all notification tones" on={ns.sounds} onToggle={() => { setN('sounds', !ns.sounds, 'd_sounds'); save('sounds_enabled', !ns.sounds); }} />

    {ns.sounds && (<>
      <div style={{ padding: '10px 14px', background: T.sf2, borderRadius: 'var(--radius-md)', border: `1px solid ${T.bd}`, marginBottom: 6 }}>
      ...
      {([
        ...
        <div key={opt.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', background: T.sf2, borderRadius: 'var(--radius-md)', border: `1px solid ${T.bd}`, marginBottom: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: ns.desktopPerm !== 'granted' ? 8 : 0 }}>
        ...
        {ns.desktopPerm !== 'granted' && ns.desktopPerm !== 'denied' && (
          <button onClick={requestDesktopPermission}
            style={{ padding: '6px 14px', borderRadius: 'var(--radius-md)', border: 'none', background: T.ac, color: '#000', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
            Allow
          </button>
        )}
      </div>
    ...
    {ns.dnd && (
      <div style={{ padding: '8px 14px', background: 'rgba(237,66,69,0.08)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(237,66,69,0.2)', marginBottom: 6, fontSize: 11, color: '#ed4245', display: 'flex', alignItems: 'center', gap: 6 }}>
        DND is active — all notifications suppressed except DM @mentions.
      </div>
    )}
    <div style={{ padding: '14px', background: T.sf2, borderRadius: 'var(--radius-md)', border: `1px solid ${T.bd}`, marginBottom: 6 }}>
    ...
    {notifServers.map(sv => {
      ...
      return (
        <div key={sv.id} style={{ padding: '10px 14px', background: T.sf2, borderRadius: 'var(--radius-md)', border: `1px solid ${T.bd}`, marginBottom: 6 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <div style={{ width: 28, height: 28, borderRadius: 'var(--radius-md)', background: sv.icon_url ? 'transparent' : `${ta(T.ac,'33')}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: T.ac, overflow: 'hidden', flexShrink: 0 }}>
