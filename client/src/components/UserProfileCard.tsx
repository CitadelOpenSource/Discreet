...
export function UserProfileCard({
  ...
  return ReactDOM.createPortal(
    <div ref={cardRef} data-testid="user-profile" data-component="UserProfileCard" style={{ position: 'fixed', left, top, zIndex: 10000, width: 300, background: T.sf, border: `1px solid ${T.bd}`, borderRadius: 'var(--border-radius)', boxShadow: '0 12px 40px rgba(0,0,0,0.5)', overflow: 'hidden', fontFamily: 'var(--font-primary)' }}>
      {/* Banner + Avatar */}
      <div style={{ height: 60, background: `linear-gradient(135deg, ${ta(T.ac,'44')}, ${ta(T.ac2 ?? T.ac,'44')})`, position: 'relative' }}>
        ...
        {!isSelf && (
          <div style={{ position: 'absolute', top: 8, right: 8, display: 'flex', gap: 4 }}>
            <div onClick={toggleMute} title={muted ? 'Unmute' : 'Mute'} style={{ width: 28, height: 28, borderRadius: 6, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: muted ? T.err : T.mt }}>{muted ? <I.VolX s={16} /> : <I.Vol s={16} />}</div>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '28px 16px 8px' }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: T.tx, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {(user.platform_role === 'admin' || user.platform_role === 'dev') && (
            <span title="This user is a Discreet staff member" style={{ fontSize: 10, fontWeight: 700, color: '#00D4AA', display: 'inline-flex', alignItems: 'center', gap: 3, background: 'rgba(0,212,170,0.1)', padding: '1px 6px', borderRadius: 4 }}><I.ShieldCheck s={11} /> Staff</span>
          )}
        </div>
      ...
      <div style={{ padding: 12, minHeight: 80, maxHeight: 200, overflowY: 'auto' }}>
        ...
        {tab === 'note' && (<>
          ...
          <textarea value={note} onChange={e => saveNote(e.target.value)} placeholder="Add a note about this user..." style={{ width: '100%', minHeight: 60, background: T.bg, border: `1px solid ${T.bd}`, borderRadius: 6, color: T.tx, fontSize: 12, padding: 8, resize: 'vertical', fontFamily: 'var(--font-primary)', boxSizing: 'border-box' }} />
        </>)}
      ...
      {isBot && isServerOwner && onConfigBot && (
        <div style={{ padding: '8px 12px', borderTop: `1px solid ${T.bd}` }}>
          <button onClick={() => { onConfigBot({ bot_user_id: userId, username: user?.username, display_name: user?.display_name || user?.username, persona: user?.persona || 'general' }); onClose(); }} className="pill-btn" style={{ width: '100%', padding: '9px 0', background: 'linear-gradient(135deg,#7289da,#5865F2)', color: '#fff', border: 'none', borderRadius: 'var(--radius-md)', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>🤖 Configure Bot</button>
        </div>
      )}
...
function SelfStatusEdit({ user, api: apiRef }: SelfStatusEditProps) {
  ...
  const [onlineStatus, setOnlineStatus] = useState(localStorage.getItem('d_manual_status') || 'online');

  if (!editing) return (
    <div style={{ padding: '0 16px 8px' }}>
      <button onClick={() => setEditing(true)} className="pill-btn" style={{ width: '100%', padding: '8px 0', background: `linear-gradient(135deg,${ta(T.ac,'22')},${ta(T.ac,'22')})`, color: T.ac, border: `1px solid ${ta(T.ac,'44')}`, borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>Edit Profile &amp; Status</button>
    </div>
  );

  return (
    <div style={{ padding: '0 16px 8px' }}>
      <div style={{ background: T.bg, borderRadius: 'var(--radius-md)', padding: 10, border: `1px solid ${T.bd}` }}>
        ...
        <label style={{ fontSize: 10, fontWeight: 700, color: T.mt, textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>Status</label>
        <div style={{ display: 'flex', gap: 4, marginBottom: 8 }}>
          {[
            { id: 'online', label: 'Online', color: '#3ba55d' },
            { id: 'idle', label: 'Idle', color: '#faa61a' },
            { id: 'dnd', label: 'DND', color: '#ed4245' },
            { id: 'invisible', label: 'Invisible', color: '#747f8d' },
          ].map(s => (
            <button key={s.id} onClick={() => setOnlineStatus(s.id)}
              style={{ flex: 1, padding: '4px 0', borderRadius: 4, border: `1px solid ${onlineStatus === s.id ? s.color : T.bd}`, background: onlineStatus === s.id ? `${s.color}22` : 'transparent', color: onlineStatus === s.id ? s.color : T.mt, fontSize: 10, fontWeight: 600, cursor: 'pointer' }}>
              {s.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <button onClick={async () => {
            ...
            localStorage.setItem('d_manual_status', onlineStatus);
            ...
            if (apiRef.ws && apiRef.ws.readyState === 1) {
              ...
              apiRef.ws.send(JSON.stringify({ type: 'status_change', status: onlineStatus }));
            }
            setEditing(false);
